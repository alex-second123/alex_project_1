...
PRIORITIES = ('closed', 'low', 'normal', 'high', 'available to check out', 'order placed', 'unable to processs')
...
def error_if_helprequest_not_found(helprequest_id):
    if helprequest_id not in data['helprequests']:
        message = "No equipment here with with ID: {}".format(helprequest_id)
        abort(404, message=message)


def error_if_helprequest_item_not_available(helprequest_id):
    if helprequest_id not in data['helprequests']:
        message = " ID: {} cannot be processed, item is checked out".format(helprequest_id)
...
# of that list.
def collection_of_equipment_list_as_html(collection_of_equipment):
    return render_template(
        'helprequests+microdata+rdfa.html',
        collection_of_equipment=collection_of_equipment,
        priorities=PRIORITIES)
...
#PID_checker not in use
def pid_is_invalid(pid):
    pid = int(pid)
    if len(pid) != 9:
        raise ValueError('PID is incorrect')
...
for arg in ['from', 'Equipment', 'PID', 'title', 'description']:
...
update_helprequest_parser.add_argument(
    'priority', type=str, default=PRIORITIES.index('normal'))
update_helprequest_parser.add_argument(
...
class HelpRequest(Resource):
    ...
    #respond with a 404 error if item is not available for checkout, otherwise same as above
    # with the updated HTML representation.
    def patch(self, helprequest_id):
        error_if_helprequest_not_found(helprequest_id)
        error_if_helprequest_item_not_available()
...
class HelpRequestList(Resource):
    ...
    def post(self):
        ...
        helprequest['priority'] = PRIORITIES.index('reservation placed!')
...
#class to allow the auth user to edit and add equipment to the resource
class EquipmentList(Resource):
  ...
  def patch(self, helprequest_id):
        ...
        if len(update['comment'].strip()) > 0:
            helprequest.setdefault('comments', []).append(update['comment'])
        return make_response(render_helprequest_as_html(helprequest), 200)
    ...
    '''#call cal, function to post the reservation to each date on the Calendar with both the start and end date
        def PostReservationToDate(week, weekDay, helprequest, helprequest_id, date_start, date_end):
            for weekday in week:
                if weekDay.contains helprequest:
                    for date_start and date_end in helprequest:
                        return something (id at the start date, id at the week date)
         '''
...
'''#function to eventually allow auth user to add equip
class AddEquipmentRequestAsJSON(Resource):
    def get(self):
        return data
'''
...
api.add_resource(EquipmentList, '/request/all_equip')
